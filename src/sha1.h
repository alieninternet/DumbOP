...
class SHA1 {
 private:
   static void transform(unsigned long state[5], unsigned char buffer[64]);
   static void init(SHA1_CTX* context);
   static void update(SHA1_CTX* context, unsigned char * data, 
		      unsigned int len);
   static void final(unsigned char digest[20], SHA1_CTX* context);
   
 public:
   static String SHA1::generate(String line);
};
