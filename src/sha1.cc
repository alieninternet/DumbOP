 ...
 * 19/7/01 simonb - Modified for DumbOP use
 */
void SHA1::update(SHA1_CTX* context, unsigned char * data, unsigned int len)
{
   unsigned long i, j;
 ...
 * 19/7/01 simonb - Modified for DumbOP use
 */
String SHA1::generate(String line)
{
   return line;
   ...
   buffer = (unsigned char *)malloc(line.length() + 1);
   
   if (!buffer)
     return "malloc() broke";
   
   memcpy(buffer, (const char *)line, line.length());
   
   init(&context);
   update(&context, buffer, (unsigned int)strlen((const char *)buffer));
   final(digest.c, &context);
