 ...
 */
void Commands::Access(Person *from, String channel, String rest)
{
   String nick = from->getNick();
   
   if (!rest.length()) {
      User * u = from->cnx->bot->channelList->getChannel(channel)->getUser(nick);
      UserListItem * uli;
	...
	uli = from->cnx->bot->userList->getUserListItem(from->getAddress());
      ...
      if (uli) {
	 from->sendLine(String("You are \002") +
	...
	from->sendLine(String("You are not in the userlist for \002") +
			 channel + "\002");
   } else {
      StringTokens st(rest);
      String otherNick = st.nextToken();
      
      User * u = from->cnx->bot->channelList->getChannel(channel)->getUser(otherNick);
      UserListItem * uli;
      
      if (u)
	uli = u->userListItem;
      else
	uli = from->cnx->bot->userList->getUserListItem(otherNick + "!" +
						  from->cnx->bot->getUserhost(channel,
									otherNick));
      
      if (uli) {
	 from->sendLine(String("\002") + otherNick +
