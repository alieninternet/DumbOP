 ...
 */
void Commands::Register(Person *from,
			    String channel, String rest)
{
   // Is this secure enough to continue?
   if (Utils::isChannel(channel)) {
      from->sendLine("For security reasons you should not use the REGISTER command on a channel.");
      return;
   }
   ...
   if (!word.length()) {
      from->sendLine("You must supply a password!");
      return;
   }
   
   // Are we registering a channel or not?
   if (Utils::isChannel(word)) {
      from->sendLine(String("You cannot register a channel with this command."));
   } else {
      // Make sure we do not have a duplicate!
      if (from->cnx->bot->userList->isNickInUserList(from->getNick())) {
	 from->sendLine(String("Your nickname is already registered!"));
	 return;
      }
