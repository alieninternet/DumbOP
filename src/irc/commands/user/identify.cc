 ...
 */
void Commands::Identify(Person *from,
			 String channel, String rest)
{
   // Is this secure enough to continue?
   if (Utils::isChannel(channel)) {
      from->sendLine("For security reasons you should not use the IDENTIFY command on a channel.");
      return;
   }
   ...
   if (!word.length()) {
      from->sendLine("You must supply a password to identify yourself!");
      return;
   }
    ...
    * well..
    */
   if (from->cnx->bot->userList->identify(from->getNick(), word)) {
      // Tell the user they have been identified correctly
      from->sendLine(String("You have been identified as \002") + 
		       from->getNick() + String("\002."));
   } else {
      // Tell the user they were not identified
      from->sendLine("Nickname/Password mismatch - You have not been identified.");
   }
}
